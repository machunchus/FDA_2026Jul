...
def procesar_diagnostico_rois(archivos_subidos, opcion_rotar, prop_sg_y, prop_sg_x, poly_sg, factor_reduccion, freq_roi):
    ...
    if rot_sel is not None: 
        img_0 = cv2.rotate(img_0, rot_sel)
    ...
    for i in ref_indices:
        ...
        if rot_sel is not None: 
            img_bgr = cv2.rotate(img_bgr, rot_sel)
        ...
        anchos_sospechosos = []
        for b_izq, b_der, _ in lista_centros_x:
            ancho_roi = b_der - b_izq
            if ancho_roi < ancho_px * 0.01 or ancho_roi > ancho_px * 0.06:
...
if archivos_subidos:
    ...
    # VALIDACIÓN 5: Verificar formato de nombres de archivo
    nombres_invalidos = []
    for f in archivos_ordenados:
        try:
            datetime.strptime(f.name.rsplit('.', 1)[0], "%Y-%m-%d_%H-%M-%S")
        except ValueError:
            nombres_invalidos.append(f.name)
    
    if nombres_invalidos:
        st.error(f"❌ {len(nombres_invalidos)} archivo(s) con formato de nombre incorrecto. Esperado: YYYY-MM-DD_HH-MM-SS")
        with st.expander("Ver archivos con problemas"):
            for nombre in nombres_invalidos[:10]:  # Mostrar solo los primeros 10
                st.write(f"- {nombre}")
        st.stop()
    ...
    if n_rois_base == 0:
        st.error("❌ No se detectaron ROIs. Ajustar parámetros de detección (ventanas S-G, umbral) y reintentar.")
        st.stop()
    ...
    if st.button("▶️ Lanzar Extracción de Intensidades", use_container_width=True):
        if n_rois_base > 0:
            ...
            # VALIDACIÓN 7: Ya validamos los nombres arriba, ahora solo parseamos
            ...
            # VALIDACIÓN 8: Contador de valores cero o NaN
            ceros_azul = 0
            valores_invalidos = 0
            ...
            for idx, archivo in enumerate(archivos_ordenados):
                ...
                if rot_sel is not None: 
                    frame_bgr = cv2.rotate(frame_bgr, rot_sel)
                ...
                for r_idx, cx in enumerate(centros_x_act):
                    ...
                    roi_v = c_v[y1:y2, x1:x2]
                    roi_a = c_a[y1:y2, x1:x2]
                    
                    # VALIDACIÓN 9: Verificar que el ROI no esté vacío
                    if roi_v.size == 0 or roi_a.size == 0:
                        valores_invalidos += 1
                        h_verde[idx, r_idx] = np.nan
                        h_azul[idx, r_idx] = np.nan
                    else:
                        val_v = func_est(roi_v)
                        val_a = func_est(roi_a)
                        h_verde[idx, r_idx] = val_v
                        h_azul[idx, r_idx] = val_a
    ...
    if st.session_state.procesado:
        ...
        # VALIDACIÓN 11: Manejo robusto de división por cero (usar np.nan en lugar de 1e-6)
        with np.errstate(divide='ignore', invalid='ignore'):
            r_crudo = np.divide(g_crudo, a_crudo, out=np.full_like(g_crudo, np.nan), where=a_crudo!=0)
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            r0 = np.divide(st.session_state.g0, st.session_state.a0, out=np.full_like(st.session_state.g0, np.nan), where=st.session_state.a0!=0)
            r_norm = np.divide(r_crudo, r0, out=np.full_like(r_crudo, np.nan), where=r0!=0)
        
        # 3. Aplicar Filtro Temporal S-G y Cutoff
        mask_cutoff = t >= t_cutoff
        dt_prom = np.mean(np.diff(t)) if len(t) > 1 else 1.0
        
        # VALIDACIÓN 12: Asegurar ventana impar correctamente
        w_cin_temp = w_sg_cin if w_sg_cin <= num_img else num_img
        if w_cin_temp % 2 == 0:
            w_cin_temp -= 1
        w_cin = max(5, w_cin_temp)  # Mínimo 5
        
        poly_cin = min(poly_sg_cin, w_cin - 1)
        ...
        if np.sum(mask_cutoff) >= w_cin:
            for r in range(n_r):
                datos_g = g_norm[mask_cutoff, r]
                datos_a = a_norm[mask_cutoff, r]
                datos_r = r_norm[mask_cutoff, r]
                
                # VALIDACIÓN 13: Verificar que no haya todos NaN
                if np.all(np.isnan(datos_g)) or np.all(np.isnan(datos_a)) or np.all(np.isnan(datos_r)):
                    st.warning(f"⚠️ ROI {r+1}: Todos los valores post-cutoff son NaN. No se calculará velocidad.")
                    continue
                
                dg = savgol_filter(datos_g, w_cin, poly_cin, deriv=1, delta=dt_prom)
                da = savgol_filter(datos_a, w_cin, poly_cin, deriv=1, delta=dt_prom)
                dr = savgol_filter(datos_r, w_cin, poly_cin, deriv=1, delta=dt_prom)
            ...
            st.warning(f"️ Solo {np.sum(mask_cutoff)} puntos post-cutoff, insuficientes para ventana de {w_cin}. Ajustar cutoff o ventana temporal.")
        ...
        html_cabecera = [
            ...
            "</head>",
            "<body>",
            "    <h1>📊 Reporte Cinético FDA - Matriz 3x4</h1>"
        ]
        html_content = "\n".join(html_cabecera) + "\n"
        ...
        for i, fila in enumerate(matriz_figuras):
            ...
            for fig in fila:
                # VALIDACIÓN 14: CDN en todas las filas para HTML standalone
                fig_html = fig.to_html(full_html=False, include_plotlyjs="cdn")
                html_content += f"<div class='col'>{fig_html}</div>\n"
            html_content += "</div>\n"
